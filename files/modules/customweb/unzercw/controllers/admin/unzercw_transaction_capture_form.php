...
class unzercw_transaction_capture_form extends oxAdminDetails
{
	/**
		...
		if ($orderId != '-1' && isset( $orderId)) {
			...
			if (UnzerCwHelper::isUnzercwPaymentMethod($paymentId)) {
				...
				foreach ($transaction->getTransactionObject()->getUncapturedLineItems() as $item) {
					...
					$uncapturedLineItems[] = array(
						'name' => UnzerCwHelper::toDefaultEncoding($item->getName()),
						'sku' => UnzerCwHelper::toDefaultEncoding($item->getSku()),
						'type' => $item->getType(),
				...
				return "unzercw_transaction_capture_form.tpl";
			}
		}
	...
	public function capture()
	{
		...
		try {
			...
			return 'unzercw_order_transactions?oxid=' . $this->getEditObjectId();
		} catch (Exception $e) {
